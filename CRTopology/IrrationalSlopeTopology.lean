...
lemma disjoint_nhs_gen (z1 : ℚ+)(z2 : ℚ+)(hz1hz2 : z1 ≠ z2):  ∃ ε₁ ε₂ : ℝ , (nhs_dit θ ε₁ z1 ∩ nhs_dit θ ε₂ z2 = ∅) ∧ (ε₁ > 0) ∧ (ε₂ > 0) := by
  match B_dijoint_ball_construct (z1.x - z1.y/θ) (z2.x - z2.y/θ) (distinct_points_z1_z2 θ hθ z1 z2 hz1hz2) with
  |⟨a,b ,hab,ha,hb⟩ =>
  match B_dijoint_ball_construct (z1.x - z1.y/θ) (z2.x + z2.y/θ) (distinct_points_z1_z2_diff θ hθ z1 z2 hz1hz2) with
  |⟨c,d,hcd,hc,hd⟩ =>
  have hθneg : Irrational (-θ) := Irrational.neg hθ
  match B_dijoint_ball_construct (z1.x - z1.y/(-θ)) (z2.x + z2.y/(-θ)) (distinct_points_z1_z2_diff (-θ) hθneg z1 z2 hz1hz2) with
  |⟨e,f,hef,he,hf⟩ =>
  rw[div_neg,div_neg,sub_neg_eq_add,← sub_eq_add_neg] at hef

  match B_dijoint_ball_construct (z1.x - z1.y/(-θ)) (z2.x - z2.y/(-θ)) (distinct_points_z1_z2 (-θ) hθneg z1 z2 hz1hz2) with
  |⟨g,h,hgh,hg,hh⟩ =>
  sorry
